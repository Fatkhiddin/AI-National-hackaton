...
import logging
from .models import (
    TelegramAccount, Contact, Chat, Message, AIIntegration,
    AutoReplyRule, AutoReplyLog, AIProvider, AIAssistant,
    ConversationSummary, CRMProvider, PropertySearchLog, PropertyInterest
)
...
def dashboard(request):
    ...
    return render(request, 'telegramai/dashboard.html', context)
...
def telegram_accounts(request):
    ...
    return render(request, 'telegramai/telegram_accounts.html', context)
...
def add_telegram_account(request):
    ...
    if request.method == 'POST':
        ...
        if not all([api_id, api_hash, phone_number, session_name]):
            ...
            return render(request, 'telegramai/add_telegram_account.html')
        ...
        try:
            ...
            return redirect('telegramai:verify_telegram_account')
        except Exception as e:
...
def verify_telegram_account(request):
    ...
    if account_id:
        try:
            ...
            return redirect('telegramai:telegram_accounts')
    else:
        telegram_account_id = request.session.get('telegram_account_id')
        if not telegram_account_id:
            messages.error(request, 'Telegram account topilmadi.')
            return redirect('telegramai:add_telegram_account')
        ...
        if action == 'send_code' or step == 'send_code':
            try:
                ...
                if result['success']:
                    ...
                    return render(request, 'telegramai/verify_telegram_account.html', {
...
def telegram_account_settings(request, account_id):
    ...
    return render(request, 'telegramai/telegram_account_settings.html', {
        'telegram_account': telegram_account,
...
def delete_all_contacts(request, account_id):
    ...
    try:
        ...
        if result_wrapper.get('success'):
            ...
            if result.get('success'):
                return JsonResponse({'success': True, 'message': result.get('message'), 'deleted_count': result.get('deleted_count', 0)})
            else:
                return JsonResponse({'success': False, 'error': result.get('error', 'Noma\'lum xatolik')})
        else:
            return JsonResponse({'success': False, 'error': result_wrapper.get('error', 'Noma\'lum xatolik')})
    except Exception as e:
...
def delete_all_chats(request, account_id):
    ...
    try:
                    ...
                    for dialog in dialogs:
                        ...
                        try:
                            if getattr(entity, "megagroup", False) or getattr(entity, "broadcast", False):
                                ...
                                try:
                                    await manager.client(DeleteHistoryRequest(peer=entity, max_id=0, revoke=True))
                                    deleted_count += 1
                                except Exception:
                                    deleted_count += 1
                        except Exception:
        ...
        if result_wrapper.get('success'):
            result = result_wrapper.get('result', {})
            if result.get('success'):
                return JsonResponse({'success': True, 'message': result.get('message'), 'deleted_count': result.get('deleted_count', 0), 'left_count': result.get('left_count', 0)})
            else:
                return JsonResponse({'success': False, 'error': result.get('error', 'Noma\'lum xatolik')})
        else:
            return JsonResponse({'success': False, 'error': result_wrapper.get('error', 'Noma\'lum xatolik')})
        ...
        async def update_profile_async():
            try:
                ...
                try:
                    ...
                    full = await manager.client(GetFullUserRequest(me.id))
                    updated_fields = []
                    if (first_name != (me.first_name or '') or 
                        last_name != (me.last_name or '') or 
                        bio != (full.full_user.about or '')):
                        ...
                        updated_fields.extend(['Ism', 'Familiya', 'Bio'])
                    if username != (me.username or ''):
                        try:
                    ...
                    message = f'Yangilandi: {", ".join(updated_fields)}' if updated_fields else 'Hech qanday o\'zgarish kiritilmadi'
...
def delete_telegram_account(request, account_id):
    """Delete Telegram account completely"""
    telegram_account = get_object_or_404(TelegramAccount, id=account_id, user=request.user)
    ...
    try:
        ...
        return JsonResponse({'success': True, 'message': f'Account {phone} butunlay o\'chirildi!'})
    except Exception as e:
        return JsonResponse({'success': False, 'error': str(e)})
...
def contacts_list(request):
    ...
    return render(request, 'telegramai/contacts_list.html', context)
...
def import_contacts(request):
    ...
    if request.method == 'POST':
        ...
        if not account_id:
            ...
            return redirect('telegramai:import_contacts')
        ...
        temp_file_path = os.path.join(media_root, unique_filename)
        ...
        return redirect('telegramai:contacts_list')
    
    telegram_accounts = TelegramAccount.objects.filter(user=request.user, is_active=True)
    return render(request, 'telegramai/import_contacts.html', {
        'telegram_accounts': telegram_accounts
    })
...
def sync_telegram_contacts(request, account_id):
    ...
    try:
        ...
        if result['success']:
            return JsonResponse({'success': True, 'message': f'{result["synced"]} ta yangi kontakt sinxronlashtirildi!'})
        else:
            return JsonResponse({'success': False, 'message': f'Xatolik: {result["error"]}'})
    except Exception as e:
...
def fix_phone_numbers(request, account_id):
    ...
    try:
        ...
        return JsonResponse({'success': True, 'message': f'{fixed_count} ta telefon raqami to\'g\'irlandi', 'fixed': fixed_count})
    except Exception as e:
        return JsonResponse({'success': False, 'message': f'Xatolik: {str(e)}'})
...
def reset_contacts_status(request, account_id):
    ...
    try:
        ...
        return JsonResponse({'success': True, 'message': f'{updated_count} ta kontakt holati reset qilindi', 'reset': updated_count})
    except Exception as e:
        return JsonResponse({'success': False, 'message': f'Xatolik: {str(e)}'})
...
def export_contacts(request):
    ...
    headers = ["№", "Ism", "Familiya", "To'liq ism", "Telefon", "Username", "Account", "Telegram mavjud", "Qo'shilgan", "Premium", "Bot", "Yaratilgan"]
...
def chats_list(request):
    """List chats for selected Telegram account"""
    telegram_accounts = TelegramAccount.objects.filter(user=request.user, is_active=True)
    ...
    if request.GET.get('refresh') == '1' and telegram_account:
        try:
                ...
                try:
                    with transaction.atomic():
                        existing_chat_ids = set(
                            Chat.objects.filter(telegram_account=telegram_account).values_list('chat_id', flat=True)
                        )
                        chats_to_create = []
                        chats_to_update = []
                        ...
                        for chat_data in chats_data:
                            ...
                            if chat_id not in existing_chat_ids:
                                chats_to_create.append(Chat(
                                    ...
                                    chat_type='private' if chat_data.get('is_user') else 'group' if chat_data.get('is_group') else 'channel',
                                    member_count=0,
                        ...
                        for chat_data in chats_to_update:
                            Chat.objects.filter(telegram_account=telegram_account, chat_id=chat_data['id']).update(
                                title=chat_data.get('title', ''),
                                username=chat_data.get('username', '') or '',
                                avatar=chat_data.get('avatar', None),
                            )
                        
                        chat_objects = {c.chat_id: c for c in Chat.objects.filter(
                            telegram_account=telegram_account,
                            chat_id__in=[cd['id'] for cd in chats_data]
                        )}
                        existing_message_ids = set(
                            Message.objects.filter(telegram_account=telegram_account).values_list('message_id', flat=True)
                        )
    ...
    ).order_by('-updated_at') if telegram_account else Chat.objects.none()
    ...
    return render(request, 'telegramai/chats_list.html', context)
...
def chats_list_new(request):
    ...
    context = {'accounts': accounts}
    return render(request, 'telegramai/chats_list_new.html', context)
...
def chats_detail(request, account_id, chat_id):
    ...
    db_messages = Message.objects.filter(chat=chat, telegram_account=telegram_account).order_by('date')
        ...
        try:
            ...
            if result.get('success'):
                ...
                if isinstance(coro_res, dict) and coro_res.get('success'):
                    ...
                    with transaction.atomic():
                        existing_message_ids = set(
                            Message.objects.filter(chat=chat, telegram_account=telegram_account).values_list('message_id', flat=True)
                        )
                        new_messages = []
    ...
    return render(request, 'telegramai/chat_detail.html', context)


@login_required
def chat_messages(request, account_id, chat_id):
    """View messages in a specific chat (legacy)"""
    chat = get_object_or_404(Chat, chat_id=chat_id, telegram_account__user=request.user)
    ...
    if request.method == 'POST':
        ...
        return redirect('telegramai:chat_messages', account_id=account_id, chat_id=chat_id)
    
    chat_msgs = Message.objects.filter(chat=chat).order_by('-date')
    paginator = Paginator(chat_msgs, 50)
    page_number = request.GET.get('page')
    page_obj = paginator.get_page(page_number)
    
    context = {'chat': chat, 'page_obj': page_obj}
    return render(request, 'telegramai/chat_messages.html', context)
...
def send_message_to_chat(request, account_id, chat_id):
    ...
    telegram_account = get_object_or_404(TelegramAccount, id=account_id, user=request.user)
    chat = get_object_or_404(Chat, chat_id=chat_id, telegram_account=telegram_account)
    ...
    try:
        ...
        if result.get('success'):
            ...
            return JsonResponse({'success': True, 'message': 'Xabar yuborildi', 'message_id': result['message_id']})
        else:
...
def telegram_web_client(request):
    ...
    if not account_id:
        telegram_accounts = TelegramAccount.objects.filter(user=request.user, is_active=True)
        return render(request, 'telegramai/telegram_web_client.html', {
            'telegram_accounts': telegram_accounts,
    ...
    return render(request, 'telegramai/telegram_web_client.html', context)
...
def get_chat_messages_api(request, account_id, chat_id):
    ...
    try:
        ...
        if result_wrapper.get('success'):
                ...
                try:
                    with transaction.atomic():
                        existing_ids = set(Message.objects.filter(
                            telegram_account=telegram_account, chat=chat
            ...
            return JsonResponse({'success': False, 'error': result_wrapper.get('error')})
    except Exception as e:
        return JsonResponse({'success': False, 'error': str(e)})
            ...
            try:
                ...
                try:
                    if await manager.is_authorized():
                        ...
                        details_data = {}
                        for attr in ['premium', 'verified', 'scam', 'fake']:
                            if hasattr(entity, attr):
                                details_data[attr] = getattr(entity, attr)
...
def get_chats_api(request, account_id):
    ...
    try:
        account = TelegramAccount.objects.get(id=account_id, user=request.user)
        chats = Chat.objects.filter(telegram_account=account).order_by('-updated_at')
        chats_data = [{
            'id': chat.id,
            'chat_id': chat.chat_id,
            'title': chat.title,
...
def ai_integration(request):
    ...
    if request.method == 'POST':
        ...
        if account_id and provider and model_name:
            ...
            return redirect('telegramai:ai_integration')
    ...
    return render(request, 'telegramai/ai_integration.html', context)
...
def send_ai_message(request, account_id, chat_id):
    ...
    try:
        ai_integration = AIIntegration.objects.get(telegram_account=telegram_account, is_active=True)
    except AIIntegration.DoesNotExist:
        return JsonResponse({'success': False, 'error': 'Bu account uchun AI integration topilmadi'})
    ...
    try:
        ...
        if ai_response:
            ...
            if result['success']:
                return JsonResponse({'success': True, 'message': 'AI xabar yuborildi', 'ai_response': ai_response})
            else:
                return JsonResponse({'success': False, 'error': result['error']})
        else:
            return JsonResponse({'success': False, 'error': 'AI javob yarata olmadi'})
    except Exception as e:
        return JsonResponse({'success': False, 'error': str(e)})
...
def ai_provider_setup(request, provider):
    ...
    valid_providers = {
        'openai': {
            'name': 'OpenAI (GPT)', 'icon': 'fas fa-robot', 'color': 'success',
            'models': ['gpt-3.5-turbo', 'gpt-4', 'gpt-4-turbo'],
            'api_docs': 'https://platform.openai.com/api-keys',
            'description': 'OpenAI GPT modellaridan foydalaning.'
        },
        'claude': {
            'name': 'Claude (Anthropic)', 'icon': 'fas fa-microchip', 'color': 'info',
            'models': ['claude-3-sonnet', 'claude-3-opus', 'claude-3-haiku'],
            'api_docs': 'https://console.anthropic.com/settings/keys',
            'description': 'Anthropic Claude AI yordamchisi.'
        },
        'gemini': {
            'name': 'Gemini (Google)', 'icon': 'fab fa-google', 'color': 'danger',
            'models': ['gemini-pro', 'gemini-pro-vision', 'gemini-1.5-pro'],
            'api_docs': 'https://makersuite.google.com/app/apikey',
            'description': 'Google Gemini AI modeli.'
        }
    }
    ...
    telegram_accounts = TelegramAccount.objects.filter(user=request.user, is_active=True)
    existing_integration = AIIntegration.objects.filter(telegram_account__user=request.user, provider=provider).first()
    ...
    return render(request, 'telegramai/ai_provider_setup.html', context)
...
def test_ai_provider_connection(provider, api_key, model_name):
    ...
    try:
        if provider == 'openai':
            ...
            data = {'model': model_name or 'gpt-3.5-turbo', 'messages': [{'role': 'user', 'content': 'Hello'}], 'max_tokens': 5}
            with httpx.Client(timeout=10.0) as client:
                response = client.post('https://api.openai.com/v1/chat/completions', headers=headers, json=data)
                if response.status_code == 200:
                    return {'success': True, 'message': 'OpenAI ulanish muvaffaqiyatli!'}
            ...
            data = {'model': model_name or 'claude-sonnet-4-20250514', 'messages': [{'role': 'user', 'content': 'Hello'}], 'max_tokens': 10}
            with httpx.Client(timeout=10.0) as client:
                response = client.post('https://api.anthropic.com/v1/messages', headers=headers, json=data)
                if response.status_code == 200:
                    return {'success': True, 'message': 'Claude ulanish muvaffaqiyatli!'}
                elif response.status_code == 401:
                    return {'success': False, 'error': 'API kalit noto\'g\'ri'}
                else:
                    return {'success': False, 'error': f'HTTP {response.status_code}'}
            ...
            with httpx.Client(timeout=10.0) as client:
                ...
                if response.status_code == 200:
                    return {'success': True, 'message': 'Gemini ulanish muvaffaqiyatli!'}
                else:
                    return {'success': False, 'error': f'HTTP {response.status_code}'}
        else:
            return {'success': False, 'error': 'Noma\'lum provider'}
    except httpx.ConnectTimeout:
        return {'success': False, 'error': 'Ulanish vaqti tugadi (timeout)'}
    except Exception as e:
...
def send_ai_message_to_chat(request, account_id, chat_id):
    ...
    try:
        ...
        return JsonResponse({'success': False, 'error': 'Bu account uchun faol AI integration topilmadi'})
        ...
        if ai_response:
            ...
            if result_wrapper.get('success'):
                ...
                if result.get('success'):
                    ...
                    return JsonResponse({'success': False, 'error': f'Telegram xabar yuborishda xatolik: {result.get("error")}'})
            else:
                return JsonResponse({'success': False, 'error': f'Xatolik: {result_wrapper.get("error")}'})
        else:
            return JsonResponse({'success': False, 'error': 'AI javob yarata olmadi.'})
    except Exception as e:
        return JsonResponse({'success': False, 'error': f'AI xabar yuborishda xatolik: {str(e)}'})
...
def ai_chat_interface(request, account_id):
    ...
    try:
        ...
        if request.method == 'POST':
            return JsonResponse({'error': 'Bu account uchun AI integration mavjud emas'}, status=400)
        messages.error(request, 'Bu account uchun AI integration mavjud emas')
        return redirect('telegramai:ai_integration')
    
    if request.method == 'POST':
        user_input = request.POST.get('message', '').strip() or request.POST.get('user_input', '').strip()
        if not user_input:
        ...
        try:
            ...
            if ai_response:
                ...
                return JsonResponse({'error': 'AI javob yaratilmadi'}, status=500)
        except Exception as e:
    ...
    return render(request, 'telegramai/ai_chat.html', context)
...
def test_ai_integration(request, account_id):
    ...
    try:
        ...
        return JsonResponse({'success': False, 'error': 'AI integration topilmadi'})
    ...
    try:
        ...
        if ai_response:
            ...
            return JsonResponse({'success': False, 'error': 'AI javob yaratilmadi'})
    except Exception as e:
        return JsonResponse({'success': False, 'error': f'Test xatolik: {str(e)}'})
...
def create_auto_reply_rule(request, account_id):
    ...
    try:
        ...
        return JsonResponse({'success': True, 'message': 'Auto-reply qoidasi yaratildi', 'rule_id': rule.id})
    except TelegramAccount.DoesNotExist:
        return JsonResponse({'success': False, 'error': 'Account topilmadi'}, status=404)
    except Exception as e:
...
def update_auto_reply_rule(request, rule_id):
    ...
    try:
        rule = AutoReplyRule.objects.get(id=rule_id, telegram_account__user=request.user)
        data = json.loads(request.body)
        
        for field in ['name', 'trigger_type', 'reply_message', 'delay_seconds',
                       'reply_once_per_user', 'work_hours_only', 'work_hours_start',
                       'work_hours_end', 'only_private_chats', 'mark_as_read',
                       'show_typing', 'typing_duration', 'is_active']:
            if field in data:
...
def toggle_auto_reply_rule(request, rule_id):
    ...
    try:
        ...
        return JsonResponse({'success': True, 'message': f'Auto-reply {status}', 'is_active': rule.is_active})
    except AutoReplyRule.DoesNotExist:
        return JsonResponse({'success': False, 'error': 'Qoida topilmadi'}, status=404)
    except Exception as e:
...
def ai_dashboard(request):
    ...
    context = {'providers': providers, 'accounts': accounts}
    return render(request, 'telegramai/ai_dashboard.html', context)
...
def ai_provider_manage(request, provider_id=None):
    ...
    if request.method == 'GET':
        ...
        try:
            ...
            return JsonResponse({'success': True, 'message': message, 'provider_id': provider.id})
        except Exception as e:
...
def ai_provider_delete(request, provider_id):
    ...
    try:
        ...
        provider.delete()
        return JsonResponse({'success': True, 'message': f'"{provider_name}" o\'chirildi'})
    except Exception as e:
...
def ai_assistant_manage(request, assistant_id=None):
    ...
    if request.method == 'GET':
        ...
        try:
            ...
            if assistant_id:
                assistant = get_object_or_404(AIAssistant, id=assistant_id, telegram_account__user=request.user)
                for field in ['name', 'model', 'system_prompt', 'is_active', 'auto_respond',
                              'only_private_chats', 'response_delay_seconds', 'mark_as_read',
                              'show_typing', 'typing_duration']:
                    if field in data:
                ...
                account = get_object_or_404(TelegramAccount, id=data['telegram_account_id'], user=request.user)
                provider = get_object_or_404(AIProvider, id=data['ai_provider_id'], user=request.user)
            ...
            return JsonResponse({'success': True, 'message': message, 'assistant_id': assistant.id})
        except Exception as e:
...
def ai_assistant_delete(request, assistant_id):
    ...
    try:
        ...
        assistant.delete()
        return JsonResponse({'success': True, 'message': f'"{assistant_name}" o\'chirildi'})
    except Exception as e:
...
def crm_integration_dashboard(request):
    ...
    recent_searches = PropertySearchLog.objects.filter(crm_provider__user=request.user)[:20]
    
    context = {
        'crm_providers': providers,
        'recent_searches': recent_searches,
    }
    return render(request, 'telegramai/crm_dashboard.html', context)
...
def crm_provider_manage(request, provider_id=None):
    ...
    if request.method == 'GET':
        ...
        try:
            ...
            if provider_id:
                provider = get_object_or_404(CRMProvider, id=provider_id, user=request.user)
                for field in ['name', 'crm_type', 'api_url', 'api_key', 'api_secret',
                              'field_mapping', 'request_template', 'extraction_prompt', 'is_active']:
                    if field in data:
...
def crm_search_properties(request):
    ...
    try:
        ...
        extraction_result = run_async(crm_service.extract_requirements_with_ai(conversation.summary_data, ai_service))
        
        if not extraction_result.get('success'):
            return JsonResponse({'success': False, 'error': 'AI tahlil xatolik: ' + extraction_result.get('error', 'Unknown')})
        
        requirements = extraction_result['requirements']
        search_result = run_async(crm_service.search_properties(requirements))
...
def property_interests_dashboard(request):
    ...
    return render(request, 'telegramai/property_interests.html', context)

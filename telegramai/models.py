...
class Contact(models.Model):
    telegram_account = models.ForeignKey(
        TelegramAccount, on_delete=models.CASCADE, related_name='contacts', null=True, blank=True
    )
...
class Message(models.Model):
    ...
    contact = models.ForeignKey(
        Contact, on_delete=models.SET_NULL, null=True, blank=True, related_name='messages'
    )
...
class AutoReplyRule(models.Model):
    ...
    excluded_users = models.TextField(
        blank=True,
        help_text="Javob bermaslik kerak userlar (username yoki user_id, har birini yangi qatorda)",
    )
...
class AIProvider(models.Model):
    ...
    user = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name='ai_providers')
...
class ConversationSummary(models.Model):
    telegram_account = models.ForeignKey(
        TelegramAccount, on_delete=models.CASCADE, related_name='conversation_summaries'
    )
    ...
    summary_data = models.JSONField(default=dict, help_text="Suhbat xulosasi JSON formatda", blank=True)

    message_count = models.IntegerField(default=0, help_text="Jami xabarlar soni")
    messages_since_summary = models.IntegerField(
        default=0, help_text="Summary yaratilganidan keyingi xabarlar"
    )
...
class CRMProvider(models.Model):
    ...
    user = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name='crm_providers')
    ...
    field_mapping = models.JSONField(
        default=dict, blank=True, help_text="CRM field mapping - Telegram summary -> CRM fields"
    )
    request_template = models.JSONField(default=dict, blank=True, help_text="API so'rov template")
    extraction_prompt = models.TextField(
        blank=True, help_text="AI ga beriluvchi prompt - suhbatdan uy talablarini JSON formatda chiqarish uchun"
    )
...
class PropertySearchLog(models.Model):
    ...
    telegram_account = models.ForeignKey(
        TelegramAccount, on_delete=models.CASCADE, related_name='property_searches'
    )
    chat_id = models.BigIntegerField(help_text="Telegram chat ID")
...
class PropertyInterest(models.Model):
    ...
    telegram_account = models.ForeignKey(
        TelegramAccount, on_delete=models.CASCADE, related_name='property_interests'
    )
    search_log = models.ForeignKey(
        PropertySearchLog, on_delete=models.CASCADE, related_name='interests', null=True, blank=True
    )

...
class AIService:
    ...
    async def _generate_openai(self, system_prompt, user_message, conversation_history, max_tokens):
        try:
    ...
    async def _generate_anthropic(self, system_prompt, user_message, conversation_history, max_tokens):
        try:
            messages = []
            if conversation_history:
                messages.extend(conversation_history)
            ...
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                if response.status_code != 200:
                    return {"success": False, "error": f"Anthropic API error: {response.status_code} - {response.text}"}
                data = response.json()
                return {
                    "success": True,
    ...
    async def _generate_google(self, system_prompt, user_message, conversation_history, max_tokens):
        try:
            contents = []
            if system_prompt:
...
JSON STRUKTURA:
{
  ...
  "kayfiyat": "issiq / sovuq / neytral / chalkash / xafa",

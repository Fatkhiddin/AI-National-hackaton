...
class TelegramManager:
    def __init__(self, telegram_account):
        ...
        self.client = TelegramClient(
            self.session_file, telegram_account.api_id, telegram_account.api_hash
        )
        ...
        try:
            if password:
                ...
                result = await self.client.sign_in(
                    phone_number, code, phone_code_hash=phone_code_hash
                )
    ...
    async def download_chat_avatar(self, chat_id):
        ...
        try:
            ...
            file_path = await self.client.download_profile_photo(
                entity, file=os.path.join(avatar_dir, f"avatar_{chat_id}.jpg")
            )
                    ...
                    try:
                        ...
                        if not created:
                            contact_obj.first_name = (
                                getattr(contact, "first_name", "") or contact_obj.first_name
                            )
                            contact_obj.last_name = (
                                getattr(contact, "last_name", "") or contact_obj.last_name
                            )
                            contact_obj.username = (
                                getattr(contact, "username", "") or contact_obj.username
                            )
                            contact_obj.phone_number = (
                                getattr(contact, "phone", "") or contact_obj.phone_number
                            )
        ...
        try:
            ...
            contacts = await get_contacts(
                Contact.objects.filter(
                    telegram_account=self.telegram_account, added_to_telegram=False
                )
            ...
            if not contacts:
                return {
                    "success": False,
                    "error": "Qo'shish uchun kontakt topilmadi yoki barcha kontaktlar allaqachon qo'shilgan",
                }
            ...
            for contact in contacts:
                ...
                if formatted_phone and len(formatted_phone) >= 10:
                    ...
                    if formatted_phone != "+" + digits_only:
                        fixed_phones.append(
                            f"{contact.name or 'Nomsiz'}: {phone} -> {formatted_phone}"
                        )
                else:
            ...
            for i in range(0, total_to_process, batch_size):
                ...
                try:
                    ...
                    for c in batch_contacts:
                        c.added_to_telegram = True
                        await save_contact(c)
                        added_count += 1
                    imported_count += len(result.imported)
                except Exception as e:
                    for c in batch_contacts:
                        if "Cannot find any entity" in str(e):
                            c.telegram_exists = False
                            c.added_to_telegram = False
                        else:
                            c.added_to_telegram = True
                        await save_contact(c)
                        added_count += 1
...
def import_contacts_from_excel(file_path, telegram_account=None):
    ...
    try:
        ...
        return {"success": False, "error": str(e), "total": 0, "successful": 0, "failed": 0}
    finally:
        if workbook:
            del workbook

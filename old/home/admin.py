...
class TelegramAccountAdmin(admin.ModelAdmin):
    ...
    # Removed list_editable to reduce field count  
    list_per_page = 25  # Smaller page size
...
class ContactAdmin(ImportExportModelAdmin):
    ...
    # Removed list_editable to reduce field count
    list_per_page = 50  # Reduced for better performance
    list_max_show_all = 200  # Reduced limit
    list_select_related = ('telegram_account',)  # Optimize queries
...
class MessageAdmin(admin.ModelAdmin):
    ...
    date_hierarchy = 'date'
    list_per_page = 25  # Smaller page size
    list_select_related = ('chat', 'contact', 'telegram_account')  # Optimize queries
...
class ContactImportHistoryAdmin(admin.ModelAdmin):
    ...
    def has_add_permission(self, request):
        return False  # Import history should be created automatically
    
    def has_change_permission(self, request, obj=None):
        return False  # Import history should not be editable
...
class ConversationSummaryAdmin(admin.ModelAdmin):
    ...
    def formatted_summary_display(self, obj):
        """Display summary in beautiful readable format"""
        from django.utils.html import format_html
        if not obj.summary_data:
        ...
        # Imkoniyatlari
        if data.get('imkoniyatlari'):
            imk = data['imkoniyatlari']
            items = []
            if imk.get('byudjet') and imk['byudjet'] != 'noma\'lum':
                items.append(f"💰 Byudjet: <strong>{imk['byudjet']}</strong>")
            if imk.get('joy_tanlovi') and imk['joy_tanlovi'] != 'noma\'lum':
                items.append(f"📍 Joy: <strong>{imk['joy_tanlovi']}</strong>")
            if imk.get('vaqt_rejasi'):
                items.append(f"⏰ Vaqt: <strong>{imk['vaqt_rejasi']}</strong>")
            
            if items:
                html_parts.append(
                    f'<div style="padding: 10px; background: #e3f2fd; margin-bottom: 10px;">'
                    f'<strong>💼 Imkoniyatlari:</strong><br>' + '<br>'.join(items) + '</div>'
                )
        
        # Qiziqishi
        if data.get('qiziqishi'):
            qiziq = [q for q in data['qiziqishi'] if q != 'noma\'lum']
            if qiziq:
                html_parts.append(
                    f'<div style="padding: 10px; background: #fff3e0; margin-bottom: 10px;">'
                    f'<strong>🏠 Qiziqishi:</strong> {", ".join(qiziq)}</div>'
                )
        
        # Muhim faktlar
        if data.get('muhim_faktlar'):
            faktlar = '<br>• '.join(data['muhim_faktlar'])
            html_parts.append(
                f'<div style="padding: 10px; background: #f3e5f5; margin-bottom: 10px;">'
                f'<strong>✅ Muhim faktlar:</strong><br>• {faktlar}</div>'
            )
        
        # Kayfiyat
        if data.get('kayfiyat'):
            kayf_map = {
                'issiq': ('😊', 'Issiq', '#4CAF50'),
                'sovuq': ('😐', 'Sovuq', '#9E9E9E'),
                'neytral': ('😌', 'Neytral', '#2196F3'),
                'chalkash': ('😕', 'Chalkash', '#FF9800'),
                'xafa': ('😠', 'Xafa', '#F44336')
            }
            kayf = data['kayfiyat']
            if kayf in kayf_map:
                emoji, text, color = kayf_map[kayf]
                html_parts.append(
                    f'<div style="padding: 10px; background: {color}20; border-left: 3px solid {color}; margin-bottom: 10px;">'
                    f'<strong>{emoji} Kayfiyat:</strong> {text}</div>'
                )
...
class PropertyInterestAdmin(admin.ModelAdmin):
    ...
    def property_data_display(self, obj):
        """Display formatted property data"""
